...
: public Widget 
{
	public:
		...
		/**
		 * Inserts text before specified line number. Text can contain
		 * multiple lines and should end with `\n' character just in front of
		 * `\0' character.
		 */
		 ...
		 * line after the last removed line, thus range of <start_line,
		 * end_line) lines is removed.
		 */
		...
		/**
		 * Returns an on-screen line number where a text specified by line
		 * number is located. Note: this number can change if the widget is
		 * resized.
		 */
	...
	protected:
		/**
		 * Struct Line saves a real line. All text added into TextView is
		 * split on `\n' character and stored into Line objects.
		 */
		struct Line
		{
			/**
			 * UTF-8 encoded text. Note: Newline character is not part of
			 * text.
			 */
			gchar *text;
			/**
